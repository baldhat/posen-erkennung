...
if __name__ == '__main__':
    output_path = "E:\\data\\rendered_scenes\\output_cl_id_1-6_p12"
    phase = "train"
    with open("{0}/{1}/metadata.pkl".format(output_path, phase), "rb") as f:
    ...
    for i, scene in enumerate(metadata):
        ...
        for image in scene['images']:
            ...
            clear_img = cv2.imread("{0}/{1}/{2}_color.png".format(output_path, phase, image_id), -1)
            ...
            depth_image = cv2.imread("{0}/{1}/{2}_depth.png".format(output_path, phase, image_id))

            mask = cv2.imread("{0}/{1}/{2}_segmentation.png".format(output_path, phase, image_id))
            binary_masks = []
            ...
            matrix_world = image['RT_world_cam']

            orig, x, y, z = get_coordinate_axes(matrix_world, intrinsics)
