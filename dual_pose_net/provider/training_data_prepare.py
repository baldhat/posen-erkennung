...
def annotate_camera_train(data_dir):
    ...
    for i, img_path in enumerate(tqdm(camera_train)):
        ...
        image = cv2.imread(img_full_path + '_color.png')
        ...
        try:
            ...
            masks, coords, class_ids, instance_ids, _, _, sizes = process_data(img_full_path, depth, subset='train')
        except:
            continue
...
def annotate_real_train(data_dir):
    ...
    for img_path in tqdm(real_train):
        ...
        if instance_ids is None:
            continue
        # Umeyama alignment of GT NOCS map with depth image
        pts, input_dis, input_rgb, scales, rotations, translations, error_messages, _ , centroids = \
            align_nocs_to_depth(masks, coords, depth, image, intrinsics, instance_ids, img_path)
        if error_messages:
            continue
        # write results
        ...
        for k in range(pts.shape[0]):
            label = class_ids[k] - 1
            r = rotations[k]
            visualize(r, translations[k] + centroids[k], image, intrinsics, (scales[:, np.newaxis] * sizes)[k])
            if label in sym_id:
                theta_x = r[0, 0] + r[2, 2]
                theta_y = r[0, 2] - r[2, 0]
                r_norm = math.sqrt(theta_x**2 + theta_y**2)
