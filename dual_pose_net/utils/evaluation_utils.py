...
def compute_combination_mAP(final_results, synset_names, degree_thresholds=[5, 10, 15], shift_thresholds=[0.1, 0.2], iou_3d_thresholds=[0.1], n_classes=6):
    ...
    for progress, result in tqdm(enumerate(final_results)):
        ...
        if gt_RTs.shape[0] != gt_class_ids.shape[0]:
            print("Error!")
            continue
    ...
    # print('ALL:')
    print(aps.shape)
    aps[-1, :, :, :] = np.mean(aps[1:n_classes+1, :, :, :], axis=0)
...
def compute_independent_mAP(final_results, synset_names, degree_thresholds=[360], shift_thresholds=[100], iou_3d_thresholds=[0.1], iou_pose_thres=0.1, use_matches_for_pose=True, n_classes=6):
    ...
    iou_3d_aps[-1, :] = np.mean(iou_3d_aps[1:n_classes+1, :], axis=0)
    ...
    for i, degree_thres in enumerate(degree_thres_list):
        for j, shift_thres in enumerate(shift_thres_list):
            ...
            pose_aps[-1, i, j] = np.mean(pose_aps[1:n_classes+1, i, j])

    for i in range(n_classes):
        print(synset_names[i + 1])
        print('3D IoU at 25: {:.1f}'.format(
            iou_3d_aps[i + 1, iou_thres_list.index(0.25)] * 100))
        print('3D IoU at 50: {:.1f}'.format(
            iou_3d_aps[i + 1, iou_thres_list.index(0.5)] * 100))
        print('3D IoU at 75: {:.1f}'.format(
            iou_3d_aps[i + 1, iou_thres_list.index(0.75)] * 100))

        print('5 degree, 2cm: {:.1f}'.format(
            pose_aps[i + 1, degree_thres_list.index(5), shift_thres_list.index(2)] * 100))
        print('5 degree, 5cm: {:.1f}'.format(
            pose_aps[i + 1, degree_thres_list.index(5), shift_thres_list.index(5)] * 100))

        print('10 degree, 2cm: {:.1f}'.format(
            pose_aps[i + 1, degree_thres_list.index(10), shift_thres_list.index(2)] * 100))
        print('10 degree, 5cm: {:.1f}'.format(
            pose_aps[i + 1, degree_thres_list.index(10), shift_thres_list.index(5)] * 100))
