...
import utils.spherical_utils
import utils.tfnp_compatibility as tfnp
...
def sphconv(inputs, filters, use_bias=True, n_filter_params=0, weight_decay=0,
    ...
    with tf.variable_scope(None, default_name='sphconv'):
        ...
        conv = utils.spherical_utils.sph_conv_batch(inputs, ker, *args, **kwargs)
        conv = conv + bias
...
def area_weights(x, invert=False):
    ...
    n = tfnp.shape(x)[1]
    phi, theta = utils.spherical_utils.sph_sample(n)
    phi += np.diff(phi)[0]/2
