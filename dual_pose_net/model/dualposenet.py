...
from model.layers import point_transformation
from model.modules import encoder, explicit_decoder, implicit_decoder
...
class DualPoseNet(object):
    def __init__(self, opts, sess):
        ...
        elif self.opts.dataset == 'realsense_data':
            self.intrinsics = np.array([
                [599.479, 0., 325.033],
                [0., 599.497, 238.601],
                [0., 0., 1.]
            ])
        elif self.opts.dataset == 'bsyn' or self.opts.dataset == 'bsyn_segmented':
            self.intrinsics = np.array([
                [622.222229, 0, 320],
                [0, 622.222229, 320],
                [0, 0, 1.]
    ...
    def setup_optimizer(self, train_decoders_only=False):
        self.learning_rate = tf.train.exponential_decay(self.opts.learning_rate, self.global_step,
                                                        self.opts.lr_decay_steps, self.opts.lr_decay_rate,
                                                        staircase=True)
        ...
        if train_decoders_only:
            trainable_update_ops = [op for op in tf.get_collection(tf.GraphKeys.UPDATE_OPS)
                              if op.name.startswith("explicit_decoder") or op.name.startswith("implicit_decoder")]
            tvars = [var for var in tf.trainable_variables()
                         if var.name.startswith("explicit_decoder") or var.name.startswith("implicit_decoder")]
        else:
            trainable_update_ops = [op for op in tf.get_collection(tf.GraphKeys.UPDATE_OPS) if op.name.startswith(
                "encoder") or op.name.startswith("explicit_decoder") or op.name.startswith("implicit_decoder")]
            tvars = all_tvars
        with tf.control_dependencies(trainable_update_ops):
            self.all_optimizers = tf.train.AdamOptimizer(self.learning_rate).minimize(
                self.loss, var_list=tvars, colocate_gradients_with_ops=True, global_step=self.global_step)
        return all_tvars
        ...
        for epoch in range(1, self.opts.training_epoch + 1):
            ...
            for batch_idx in range(fetchworker.num_batches):
                ...
                _, loss, r_loss, t_loss, s_loss, i_loss = self.sess.run(
                    [self.all_optimizers, self.loss, self.rotation_loss, self.translation_loss, self.scale_loss,
                     self.implicit_loss], feed_dict=feed_dict)
                ...
                print(
                    '[{}][{}/{}] loss: {:.5f}({:.5f}) r_loss: {:.5f}({:.5f}) t_loss: {:.5f}({:.5f}) s_loss: {:.5f}({:.5f}) i_loss: {:.5f}({:.5f})'.format(
            ...
            self.saver.save(self.sess, os.path.join(self.opts.log_dir, self.opts.model_name), epoch)

        fetchworker.shutdown()
        print('training finished !')

    def transfer_train(self):
        print('\n*********** Transfer Training of DualPoseNet ***********')

        # model & graph
        print('building model ...')
        self.allocate_placeholders()
        self.build_model()
        t_vars = self.setup_optimizer(train_decoders_only=False)
        print('model built !')
        ...
        checkpoint_path = os.path.join(
            os.path.join('log', self.opts.old_model), self.opts.old_model_name + '-' + str(self.opts.old_model_epoch))
        ...
        for epoch in range(1, self.opts.training_epoch + 1):
            ...
            if epoch % 5 == 0:
                self.saver.save(self.sess, os.path.join(
                    self.opts.log_dir, self.opts.model_name), epoch)
    ...
    def test(self):
        ...
        checkpoint_path = os.path.join(
            self.opts.log_dir, self.opts.model_name + '-' + str(self.opts.test_epoch))
        saver.restore(self.sess, checkpoint_path)

        # test
        result_pkl_list = glob.glob(
            'E:/data/segmentation_results/{}/results_*.pkl'.format(self.opts.dataset))
        ...
        for i, path in tqdm(enumerate(result_pkl_list)):
            ...
            class_ids = data['pred_class_ids']

            try:
                image = cv2.imread('E:/' + image_path + '_color.png')[:, :, :3]
            except:
                continue
            image = image[:, :, ::-1]

            if self.opts.dataset in ["bsyn", 'realsense_data', "bsyn_segmented"]:
                depth_scale = data['depth_scale']
                depth = cv2.imread('E:/' + image_path + "_depth.png", -1)
                depth = (depth / 65535) * (depth_scale[1] - depth_scale[0]) + depth_scale[0]
                depth = depth * 1000.0
            else:
                try:
                    depth = load_depth('E:/' + image_path)
                except:
            ...
            if num_instance != 0:
                for j in range(num_instance):
                    if self.opts.model in ['REAL275', 'CAMERA25']:
                        if class_ids[j] > 6:
                            continue
                    inst_mask = 255 * pred_mask[:, :, j].astype('uint8')
                    pts_ori, idx = backproject(depth, self.intrinsics, inst_mask)
                    pts_ori = pts_ori / 1000.0
                    rgb_ori = image[idx[0], idx[1], :]
                    rgb_ori = (rgb_ori - np.array([123.7, 116.8, 103.9])[np.newaxis, :]) / 255.0

                    # Remove outliers from pointcloud and image
                    # pcd = o3d.geometry.PointCloud()
                    # pcd.points = o3d.utility.Vector3dVector(pts_ori)
                    # cl, ind = pcd.remove_radius_outlier(nb_points=40, radius=0.05)
                    # pts_ori = np.asarray(pcd.select_by_index(ind).points)
                    # rgb_ori = rgb_ori[ind]
                    ...
                    for k in range(3):
                        if FLAG:
                            ...
                            pred_canonical_pts = ((pts_ori - pred_translation[np.newaxis, :]) / pred_scale) \
                                                 @ np.transpose(pred_rotation)
    ...
    def prepare_infer(self):
        # inputs
        self.init_model()
        self.opts.log_dir = 'D:/code/python/DualPoseNet/log/' + self.opts.model
    ...
    def infer(self, depth_image, color_image, masks, class_ids):
            ...
            for j in range(num_instance):
                ...
                for k in range(3):
                    if FLAG:
                        ...
                        pred_canonical_pts = (
                                                     (pts_ori - pred_translation[np.newaxis,
                                                                :]) / pred_scale) @ np.transpose(pred_rotation)
                        dis = np.linalg.norm(pred_canonical_pts, axis=1)
    ...
    def test_refine_encoder(self):
        ...
        self.learning_rate = tf.train.exponential_decay(
            1e-6, self.global_step, 100000000, 0, staircase=True)
        ...
        checkpoint_path = os.path.join(
            self.opts.log_dir, self.opts.model_name + '-' + str(self.opts.test_epoch))
        var_to_restore = [
            val for val in tf.global_variables() if ('/Adam' not in val.name) and (
                    'encoder' in val.name or 'explicit_decoder' in val.name or 'implicit_decoder' in val.name)]
        ...
        for i, path in tqdm(enumerate(result_pkl_list)):
            ...
            if self.opts.dataset in ["bsyn", 'realsense_data']:
                depth_scale = data['depth_scale']
                    ...
                    for k in range(3):
                        ...
                        if pts.shape[0] > 32:
                            ...
                            iter_rotation, iter_translation, iter_size, iter_loss = self.sess.run(
                                [self.pred_rotation, self.pred_translation, self.pred_scale, self.loss],
                                feed_dict=feed_dict)
                    ...
                    if pts.shape[0] > 32:
                        for k in range(1000):
                            _, iter_rotation, iter_translation, iter_size, iter_loss = self.sess.run(
                                [self.all_optimizers, self.pred_rotation, self.pred_translation, self.pred_scale,
